...
function injectTechPortal() {
  var inp = 'width:100%;background:#0B1D38;border:1px solid rgba(123,95,255,.15);border-radius:8px;padding:11px 14px;color:#F8F2E0;font-size:14px;outline:none;box-sizing:border-box';
  ...
  portal.innerHTML =
    ...
    '<div style="background:#0F2645;border:1px solid rgba(123,95,255,.2);border-radius:20px;padding:0;width:100%;max-width:440px;position:relative;max-height:90vh;overflow-y:auto;scrollbar-width:none;-ms-overflow-style:none">' +
  ...
  dash.style.cssText = 'display:none;position:fixed;inset:0;z-index:10002;background:#071428;overflow:hidden';
...
function renderTechDash() {
  ...
  var html = '<div style="display:grid;grid-template-columns:180px 1fr;height:100vh;background:#071428;color:#F8F2E0;font-family:\'Roboto\',sans-serif;overflow:hidden">';

  // ── SIDEBAR ──
  html += '<aside style="background:#0B1D38;border-right:1px solid rgba(123,95,255,.08);display:flex;flex-direction:column;overflow-y:auto;padding:0">';
  // Back + Close
  ...
  if(tPage === 't-queue') {
    ...
    pending.forEach(function(b) {
      html += '<div style="background:#0F2645;border:1px solid rgba(123,95,255,.15);border-radius:14px;padding:16px;margin-bottom:12px">';
    ...
    if(confirmed.length > 0) {
      ...
      confirmed.forEach(function(b) {
        ...
        html += '<div style="background:#0F2645;border:1px solid rgba(0,200,150,.15);border-radius:14px;padding:16px;margin-bottom:10px">';
        html += '<div style="display:flex;justify-content:space-between;align-items:flex-start;margin-bottom:10px">';
  ...
  // NEARBY PAGE — Centers + KTV scan
    ...
    sortedCenters.forEach(function(c) {
      ...
      html += '<div style="background:#0F2645;border:1px solid ' + borderColor + ';border-radius:14px;padding:14px;margin-bottom:10px">';
    ...
    nearbyTechs.forEach(function(tech) {
      ...
      html += '<div style="display:flex;align-items:center;gap:12px;padding:12px;background:#0F2645;border:1px solid rgba(123,95,255,.1);border-radius:12px;margin-bottom:8px">';
      ...
      html += '<div style="position:absolute;bottom:-1px;right:-1px;width:10px;height:10px;border-radius:50%;background:' + sc + ';border:2px solid #0F2645"></div></div>';
  ...
  // CLIENTS PAGE — Customer data tracking, health records, session history
    ...
    html += '<div style="background:#0F2645;border:1px solid rgba(0,200,150,.15);border-radius:10px;padding:12px;text-align:center"><div style="font-size:20px;font-weight:700;color:#00E5A8">' + allClients.length + '</div><div style="font-size:9px;color:rgba(248,242,224,.42);text-transform:uppercase;letter-spacing:1px">' + t('Kh\u00E1ch h\u00E0ng','Clients') + '</div></div>';
    html += '<div style="background:#0F2645;border:1px solid rgba(245,158,11,.15);border-radius:10px;padding:12px;text-align:center"><div style="font-size:20px;font-weight:700;color:#F59E0B">' + incompleteClients.length + '</div><div style="font-size:9px;color:rgba(248,242,224,.42);text-transform:uppercase;letter-spacing:1px">' + t('Ch\u01B0a xong','Incomplete') + '</div></div>';
    html += '<div style="background:#0F2645;border:1px solid rgba(123,95,255,.15);border-radius:10px;padding:12px;text-align:center"><div style="font-size:20px;font-weight:700;color:#9B82FF">' + (totalEarned/1000000).toFixed(1) + 'M</div><div style="font-size:9px;color:rgba(248,242,224,.42);text-transform:uppercase;letter-spacing:1px">' + t('Th\u00F9 lao','Earned') + '</div></div>';
    html += '</div>';
    ...
    if(clientSubPage === 'list') {
      ...
      sortedClients.forEach(function(cl) {
        ...
        html += '<div onclick="window._tViewClient(\'' + cl._id + '\')" style="background:#0F2645;border:1px solid ' + (isComplete?'rgba(0,200,150,.15)':'rgba(245,158,11,.2)') + ';border-radius:12px;padding:14px;margin-bottom:8px;cursor:pointer;display:flex;align-items:center;gap:12px">';
    ...
    // ── CLIENT DETAIL VIEW ──
      ...
      if(cl) {
        ...
        html += '<div style="background:#0F2645;border:1px solid rgba(0,200,150,.12);border-radius:14px;padding:16px;margin-bottom:12px">';
        ...
        // Session history
        html += '<div style="background:#0F2645;border:1px solid rgba(123,95,255,.12);border-radius:14px;padding:16px;margin-bottom:12px">';
    ...
    // ── CLIENT FORM VIEW (new/edit) ──
      ...
      var fStyle = 'width:100%;background:#0B1D38;border:1px solid rgba(123,95,255,.15);border-radius:8px;padding:10px 12px;color:#F8F2E0;font-size:13px;outline:none;box-sizing:border-box;margin-bottom:10px';
      ...
      var secStyle = 'background:#0F2645;border:1px solid rgba(123,95,255,.1);border-radius:12px;padding:14px;margin-bottom:12px';
  ...
  // ═══════════════════════════════════════════
    ...
    if(subTab === 'overview') {
      ...
      kpis.forEach(function(k){
        html += '<div style="background:#0F2645;border:1px solid rgba(123,95,255,.12);border-radius:12px;padding:14px">';
      ...
      html += '<div style="background:#0F2645;border:1px solid rgba(123,95,255,.12);border-radius:14px;padding:16px;margin-bottom:16px">';
      ...
      html += '<div style="background:#0F2645;border:1px solid rgba(123,95,255,.12);border-radius:14px;padding:16px">';
    ...
    // ── WITHDRAW ──
      ...
      var bankInp = 'width:100%;background:#0B1D38;border:1px solid rgba(123,95,255,.15);border-radius:8px;padding:10px 12px;color:#F8F2E0;font-size:13px;outline:none;box-sizing:border-box';
      ...
      html += '<button onclick="window._ktvRequestWithdraw()" style="width:100%;padding:12px;border:none;border-radius:8px;background:linear-gradient(135deg,#00C896,#00E5A8);color:#0B1D38;font-size:14px;font-weight:700;cursor:pointer">' + t('G\u1EEDi y\u00EAu c\u1EA7u r\u00FAt ti\u1EC1n','Submit Withdrawal') + '</button>';
      ...
      if(pendingW.length > 0) {
        html += '<div style="background:#0F2645;border:1px solid rgba(255,77,109,.12);border-radius:14px;padding:16px">';
    ...
    // ── Performance Gauges ──
    html += '<div style="background:#0F2645;border:1px solid rgba(123,95,255,.1);border-radius:14px;padding:16px;margin-bottom:12px">';
    ...
    // ── Personal Info ──
    html += '<div style="background:#0F2645;border:1px solid rgba(123,95,255,.08);border-radius:14px;padding:16px;margin-bottom:14px">';
    ...
    html += '<div style="background:#0F2645;border:1px solid rgba(0,200,150,.08);border-radius:14px;padding:16px;margin-bottom:14px">';
  ...
  // ══════ AFFILIATE PAGE ══════
    ...
    // Referral Code Card
    html += '<div style="background:linear-gradient(135deg,rgba(37,99,235,.08),#0F2645);border:1px solid rgba(77,166,255,.2);border-radius:16px;padding:20px;margin-bottom:16px">';
    ...
    html += '<div style="background:#0F2645;border:1px solid rgba(77,166,255,.12);border-radius:12px;padding:16px;text-align:center"><div style="font-size:24px;font-weight:700;color:#4DA6FF">'+(affData.referrals||0)+'</div><div style="font-size:10px;color:rgba(248,242,224,.35);margin-top:4px">'+t('Ng\u01B0\u1EDDi Gi\u1EDBi Thi\u1EC7u','Referrals')+'</div></div>';
    html += '<div style="background:#0F2645;border:1px solid rgba(0,200,150,.12);border-radius:12px;padding:16px;text-align:center"><div style="font-size:24px;font-weight:700;color:#00E5A8;font-family:\'Roboto Mono\',monospace">'+formatVND(affiliateComm)+'</div><div style="font-size:10px;color:rgba(248,242,224,.35);margin-top:4px">'+t('Hoa H\u1ED3ng','Commission')+'</div></div>';
    html += '<div style="background:#0F2645;border:1px solid rgba(255,184,0,.12);border-radius:12px;padding:16px;text-align:center"><div style="font-size:24px;font-weight:700;color:#FFB800">'+(affData.level||'F0')+'</div><div style="font-size:10px;color:rgba(248,242,224,.35);margin-top:4px">'+t('C\u1EA5p B\u1EADc','Level')+'</div></div>';
    html += '</div>';
    // Commission tiers
    html += '<div style="background:#0F2645;border:1px solid rgba(77,166,255,.08);border-radius:14px;padding:16px;margin-bottom:16px">';
    ...
    html += '</div>';
    // Referral history
    html += '<div style="background:#0F2645;border:1px solid rgba(77,166,255,.08);border-radius:14px;padding:16px">';
  ...
  // ══════ REVIEWS PAGE ══════
    ...
    html += '<div style="background:#0F2645;border:1px solid rgba(123,95,255,.12);border-radius:14px;padding:20px;margin-bottom:16px;text-align:center">';
    ...
    allReviews.sort(function(a,b){return (b.createdAt||'').localeCompare(a.createdAt||'');}).forEach(function(r){
      html += '<div style="background:#0F2645;border:1px solid rgba(123,95,255,.08);border-radius:12px;padding:14px;margin-bottom:8px">';
  ...
  // ══════ TRAINING PAGE ══════
    ...
    courses.forEach(function(c){
      ...
      html += '<div style="background:#0F2645;border:1px solid rgba(123,95,255,.1);border-radius:14px;padding:16px;margin-bottom:10px;display:flex;gap:14px;align-items:center">';
  ...
  // ══════ NOTIFICATIONS PAGE ══════
    ...
    autoNotifs.forEach(function(n){
      html += '<div style="background:#0F2645;border:1px solid rgba(123,95,255,' + (n.read?'.04':'.15') + ');border-radius:12px;padding:12px 14px;margin-bottom:6px;display:flex;gap:10px;align-items:flex-start;opacity:' + (n.read?'.6':'1') + '">';
